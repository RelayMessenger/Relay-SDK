...
Record version skew instead of upgrading it away. Example authoring snapshot from 2026-07-18: local `~/.local/bin/hermes` reported `0.16.0`, passed `hermes acp --check`, while upstream released `0.18.2`. Record that delta and test the installed `0.16.0`; do not run `hermes update` as part of adapter validation. Cite the [Hermes releases](https://github.com/NousResearch/hermes-agent/releases) when recording upstream state.
