...
description: >-
  Author, review, debug, validate, package, or release Relay coding-agent integrations in the relayapp developer repository. Use when adding or changing the engine catalog, Claude/Codex bundled ACP adapters, an external ACP runtime such as Hermes, shell-free process spawning, ACP initialization/session load/prompt/permission handling, Relay permission cards and replies, Claude Code or OpenClaw channel plugins, support-tier/version claims, manifests, gateway harnesses, or installed-tarball ACP smokes.
---
...
3. Read `packages/relayapp/README.md`, the target runtime's catalog/adapter code, its tests, its package manifest, and `package-lock.json` before editing.
...
| --- | --- |
| Engine names and external runtime specs | `packages/relayapp/src/engine/catalog.ts` |
| ACP process, handshake, sessions, turns, permissions | `packages/relayapp/src/engine/acp.ts` |
| Engine process-tree shutdown | `packages/relayapp/src/engine/process.ts` |
| Engine boundary types | `packages/relayapp/src/engine/types.ts` |
| Engine selection and lifecycle | `packages/relayapp/src/flags.ts`, `packages/relayapp/src/cli.ts` |
| External runtime preflight | `packages/relayapp/src/doctor.ts` |
| Durable conversation/session binding | `packages/relayapp/src/store.ts` (`SessionStore`) |
| Relay receive/turn/reply loop | `packages/relayapp/src/receive.ts` |
| Permission card, parser, timeout, durable approval | `packages/relayapp/src/permissions.ts` |
...
Choose an **ACP engine** when `relayapp start` should own the receive loop and drive one coding runtime turn over ACP stdio. A Relay conversation maps to one runtime session; Relay receives `session/update` output and answers `session/request_permission`.
...
Ask: “Who owns the session and turn loop?” If `relayapp` owns it, use an engine. If the runtime owns it and Relay is one ingress/egress transport, use a channel.

## Extend the engine catalog without runtime mutation

Keep `packages/relayapp/src/engine/catalog.ts` as the closed catalog owner. Update `ENGINE_NAMES`, `EngineName`, display labels, CLI help, flags tests, doctor output, README, and runtime tests together.
...
1. Add the adapter as an **exact** dependency in `packages/relayapp/package.json`; update `package-lock.json` with the package manager, never by hand.
...
5. Never download, update, or install the external runtime during `relayapp start`.
...
Use `engineEnv()` as an allowlist. Pass platform basics and named provider/runtime prefixes only. Keep Relay Agent Tokens, deploy credentials, GitHub tokens, npm tokens, cloud credentials, `NODE_OPTIONS`, and unrelated parent secrets out of the child environment. Add a variable only through a narrow exact name or prefix in `RELAYAPP_ENGINE_ENV`; do not add an “inherit everything” escape hatch.
...
- In `packages/relayapp/src/receive.ts`, treat Relay `401` as terminal, tell the user to pair again, throw, and dispose the engine tree.
...
Keep the engine broker and Claude channel parser behavior aligned. When changing the card or verdict grammar, update both `packages/relayapp/src/permissions.ts` and `integrations/claude-code/src/bridge.ts`, plus both test suites.
...
6. Prove ACP startup from an installed `relayapp` tarball, not the source tree. Create a disposable directory, pack `relayapp`, install that exact tarball, then run:

```sh
npm run runtime:smoke -- claude <temp-install>/node_modules/relayapp
npm run runtime:smoke -- codex <temp-install>/node_modules/relayapp
hermes --version
hermes acp --check
