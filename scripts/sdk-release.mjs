...
const pkg = JSON.parse(
  readFileSync(resolve(repoRoot, "packages/sdk/package.json"), "utf8"),
);
const expectedTag = `sdk-v${pkg.version}`;
const npm = process.platform === "win32" ? "npm.cmd" : "npm";

function checkVersionMetadata() {
  assert.equal(pkg.name, "@relaymessenger/sdk", "this workflow publishes only the SDK");
...
async function verifyRegistry() {
  checkVersionMetadata();
  const temp = mkdtempSync(join(tmpdir(), "sdk-registry-smoke-"));
  try {
    writeFileSync(
      join(temp, "package.json"),
      `${JSON.stringify({ name: "sdk-registry-smoke", private: true, type: "module" }, null, 2)}\n`,
    );
    let installed = false;
    let lastFailure = "";
    ...
    const installedPkg = JSON.parse(
      readFileSync(join(temp, "node_modules", "@relaymessenger", "sdk", "package.json"), "utf8"),
    );
    ...
    writeFileSync(
      ...
      [
        ...
        `const required = ["createRelayClient", "runPollLoop", "verifyWebhookSignature", "createFileCursorStore", "MemoryDedupe", "normalizeRelayBaseUrl"];`,
...
if (command === "check-tag") checkTag(value ?? "");
  ...
  "usage: sdk-release.mjs check-tag <sdk-vX.Y.Z> | registry-state | verify-registry",
);
