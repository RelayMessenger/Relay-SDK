...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/v1/contact_card") {
    console.log("[mock-relay] Created credential Contact Card validated");
    json(res, 200, { contact_cards: [createdCard] });
    return;
  }
  if (
    req.method === "GET" &&
...
server.on("upgrade", (req, socket, head) => {
  ...
  if (
    !token || url.pathname !== "/v1/websocket" ||
