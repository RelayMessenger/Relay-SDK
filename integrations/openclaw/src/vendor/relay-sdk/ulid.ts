 ...
 * The client mints the one it owns before it queues the request: a `msg_` id
 * per logical send, which is both the message's identity and the send's only
 * retry key. That is what makes a retry after a lost response unambiguous, so
 * the generator has to be correct rather than convenient.
 *
