...
assert.equal(new Set(entries.map(x => `${x.httpMethod} ${x.path}`)).size, 35, "Every locked HTTP operation must have SDK documentation");
...
console.log(`MCP docs: ${entries.length} SDK methods, 35 locked HTTP operations, sha256 ${source.contract}`);
