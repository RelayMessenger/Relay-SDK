...
import Relay, { BUTTONS_GUIDANCE, PAYMENT_GUIDANCE, SELECTION_GUIDANCE } from "@relaymessenger/sdk";
import { z } from "zod";
...
export const createRelayMcpServer = (options: RelayMcpServerOptions = {}): McpServer => {
  ...
  server.registerTool("execute", {
    ...
    description: "Run TypeScript or JavaScript defining async function run(client), using the initialized Relay SDK HTTP resource methods. "
      ...
      + BUTTONS_GUIDANCE + " " + SELECTION_GUIDANCE + " " + PAYMENT_GUIDANCE
      ...
      + " Create a payment request with client.paymentRequests.create({ amount, currency, description, category }), then send"
