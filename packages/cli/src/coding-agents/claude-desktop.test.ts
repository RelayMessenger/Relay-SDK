...
import agent from "./claude-desktop.js";

it("declares the Claude Desktop restart instruction", () => {
  expect(agent.start).toEqual({
    kind: "restart",
    instruction: "Restart Claude Desktop to load Relay, then ask it to read your Relay messages.",
  });
});

it("connect prints the restart instruction and waits for the bounded reply", async () => {
  ...
  const run = vi.fn(async (input: Parameters<TerminalObserver["run"]>[0]) => {
    expect(stdout.join("")).toContain("Restart Claude Desktop to load Relay, then ask it to read your Relay messages.");
  ...
  vi.useFakeTimers();
  try {
    const code = await runCLI(["connect", "claude-desktop", "--token", `rly_live_${"C".repeat(43)}`, "--yes", "--no-skill"], {
      ...
      connect: {
        sniff: async () => [{ id: "claude-desktop", label: "Claude Desktop", found: true }],
        startCommand,
