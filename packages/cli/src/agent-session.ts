...
import { DEFAULT_API_URL, STAGING_API_URL, defaultCreationApiURL, validateApiURL, validateToken } from "./config.js";
...
export async function openSavedAgentSession(input: AgentSessionInput, dependencies: AgentSessionDependencies): Promise<TerminalSessionResult> {
  ...
  const origin = validateApiURL(selected.api_url ?? defaultCreationApiURL());
