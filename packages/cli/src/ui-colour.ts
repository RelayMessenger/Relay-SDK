...
export function relayBlueForEnv(value: string, env: NodeJS.ProcessEnv): string {
  if (env.NO_COLOR) return value;
  const truecolor = env.FORCE_COLOR === "3" || /truecolor|24bit/i.test(env.COLORTERM ?? "");
  const BLUE = truecolor ? `\x1b[38;2;10;132;255m` : `\x1b[38;5;33m`;
  return pc.isColorSupported ? `${BLUE}${value}\x1b[39m` : value;
}
