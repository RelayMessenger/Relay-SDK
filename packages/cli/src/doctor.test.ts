...
const run = async (profiles: RelayConfig["profiles"], signedIn = true, profile?: string, env: NodeJS.ProcessEnv = {}) => {
  const directory = await mkdtemp(join(tmpdir(), "relay-doctor-signin-"));
  const configContext = { cwd: directory, env: { ...env, RELAY_CONFIG_PATH: join(directory, "config.json") } };
  await writeConfig({
    ...emptyConfig(),
...
describe("doctor sign-in", () => {
  ...
  it("passes with a saved agent token without console sign-in", async () => {
    const result = await run({ default: { agent_token: "rly_test" } }, false);
    expect(result.code).toBe(0);
    expect(result.stdout).toContain("Not signed in; saved agents still work");
    expect(result.stdout).not.toContain("No agents yet.");
  });

  it("passes with an environment agent token without console sign-in", async () => {
    const result = await run(emptyConfig().profiles, false, undefined, { RELAY_AGENT_TOKEN: "rly_test" });
    expect(result.code).toBe(0);
    expect(result.stdout).toContain("Not signed in; saved agents still work");
    expect(result.stdout).toContain("taken from RELAY_AGENT_TOKEN");
  });

  it.each(["", "\u0001"])("rejects an invalid environment token %j without sign-in", async (token) => {
    const result = await run(emptyConfig().profiles, false, undefined, { RELAY_AGENT_TOKEN: token });
