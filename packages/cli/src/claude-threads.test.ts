...
import { claudeThreadKey, claudeThreadsPath, openClaudeThreads } from "./claude-threads.js";
...
const home = async (): Promise<{ env: NodeJS.ProcessEnv }> => {
  const folder = await mkdtemp(join(tmpdir(), "relay-claude-threads-"));
  folders.push(folder);
...
describe("where the thread ids live", () => {
  it("keeps them beside the profile that owns the agent", async () => {
    ...
    expect(claudeThreadsPath(context)).toBe(join(dirname(claudeThreadsPath(context)), "claude-threads.json"));
    expect(dirname(claudeThreadsPath(context))).toBe(dirname(join(context.env.RELAY_CONFIG_DIR!, "relay", "config.json")));
  });

  it("gives the same chat on two Relays, or two agents, two different threads", () => {
    expect(claudeThreadKey(owner, "chat-1")).toBe("https://api.relayapp.im|agent|chat-1");
    expect(claudeThreadKey({ ...owner, apiURL: "https://api.staging.relayapp.im" }, "chat-1"))
      .not.toBe(claudeThreadKey(owner, "chat-1"));
    expect(claudeThreadKey({ ...owner, handle: "other" }, "chat-1"))
      .not.toBe(claudeThreadKey(owner, "chat-1"));
  });
});
...
describe("keeping a thread id", () => {
  it("gives back nothing for a chat this computer has never seen", async () => {
    const threads = await openClaudeThreads(owner, await home());
    expect(threads.get("chat-1")).toBeUndefined();
  });

  it("gives the id back to the next run of the bridge", async () => {
    const context = await home();
    const first = await openClaudeThreads(owner, context);
    await first.set("chat-1", "01a0-thread");
    const second = await openClaudeThreads(owner, context);
  ...
  it("replaces an id it is given a second time", async () => {
    const context = await home();
    const threads = await openClaudeThreads(owner, context);
    await threads.set("chat-1", "first-thread");
    await threads.set("chat-1", "second-thread");
    expect((await openClaudeThreads(owner, context)).get("chat-1")).toBe("second-thread");
  });

  it("leaves another agent's threads in the file alone", async () => {
    const context = await home();
    const mine = await openClaudeThreads(owner, context);
    const theirs = await openClaudeThreads({ ...owner, handle: "other" }, context);
    await theirs.set("chat-1", "their-thread");
    await mine.set("chat-1", "my-thread");
    const reopened = await openClaudeThreads({ ...owner, handle: "other" }, context);
    expect(reopened.get("chat-1")).toBe("their-thread");
    expect((await openClaudeThreads(owner, context)).get("chat-1")).toBe("my-thread");
  });
  ...
  it("writes a file a person can read, and only the owner can", async () => {
    ...
    expect(JSON.parse(await readFile(claudeThreadsPath(context), "utf8")) as unknown).toEqual({
      version: 1,
  ...
  it.each([
    ...
    expect((await openClaudeThreads(owner, context)).get("chat-1")).toBe("fresh-thread");
  });
});
