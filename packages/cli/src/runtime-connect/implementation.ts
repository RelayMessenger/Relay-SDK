...
async function safeParents(path: string): Promise<void> {
  ...
  if (process.platform === 'win32' && !privateWindowsAcl(await inspectWindowsAcl(dirname(path)), true)) fail('unsafe-directory', 'The selected directory grants another principal write access; choose an owner-controlled native context.');
...
async function snapshot(path: string, optional = false, metadata = false): Promise<Snapshot> {
  ...
  try {
    ...
    if (acl && !privateWindowsAcl(acl, false, metadata)) fail('private-file-required', 'The selected file ACL must protect credentials and prevent other principals from writing.');
...
async function safeStateDirectory(directory: string): Promise<void> {
  if (!isAbsolute(directory)) fail('state-path-required', 'Select an absolute native state directory.');
  try {
    const stat = await fs.lstat(directory);
    if (!stat.isDirectory() || stat.isSymbolicLink()) fail('unsafe-path', 'Selected runtime state must be a real directory, not a linked or non-directory path.');
...
export async function planRuntimeConnect(input: RuntimeConnectInput): Promise<RuntimeConnectPlan> {
  try {
    ...
    if (target.runtime === 'openclaw') {
      if (['constructor', 'prototype', '__proto__'].includes(target.account)) fail('account-missing', 'Select an explicit non-reserved account name.');
      ...
      if (!oldToken && target.account === 'default') {
        ...
        for (const ambient of [process.env.RELAY_AGENT_TOKEN, fromFile, configuredEnv.RELAY_AGENT_TOKEN, vars.RELAY_AGENT_TOKEN]) {
          if (ambient !== undefined && ambient !== '' && ambient !== token) fail('default-scope-required', 'Resolve the selected default account environment credential before initial binding; an occupied or unknown secret will not be replaced.');
        }
      }
      ...
      if (target.runtime === 'hermes') {
        ...
        if (yaml.errors.length || yaml.warnings.length) fail('profile-resolution-required', 'Resolve the selected Hermes YAML profile configuration before handoff.');
        ...
        if (relayConfig.enabled === false) fail('channel-disabled', 'Enable the selected Hermes Relay platform using its native configuration before handoff.');
        ...
        if (configuredToken && !env.values.RELAY_AGENT_TOKEN) fail('profile-resolution-required', 'Resolve the occupied YAML credential into the selected profile secret scope before handoff.');
        const stateDir = target.stateDir ?? env.values.RELAY_STATE_DIR;
        if (!stateDir || !isAbsolute(stateDir) || stateDir.startsWith('~')) fail('state-path-required', 'Select an explicit absolute Hermes state directory.');
        const existingStateDir = env.values.RELAY_STATE_DIR || extra.state_dir;
        if (existingStateDir && existingStateDir !== stateDir) fail('state-path-conflict', 'The selected Hermes profile already resolves another state directory; no state was moved.', true);
        if (!configuredToken) {
          validateState = async () => {
        ...
        validateState = async () => {
          try {
            ...
            if ((await fs.readdir(statePath)).length) fail('durable-state-present', 'The token-empty Claude context contains durable state; verify its identity before handoff.', true);
  ...
  try {
    ...
    try {
      if (process.platform === 'win32') {
        ...
        if (!privateWindowsAcl(acl)) fail('private-file-required', 'Could not establish a private credential ACL before writing.');
      }
