...
describe("Relay channel configuration", () => {
  ...
  it("resolves a linked folder profile and walks parents", () => {
    ...
    writeFileSync(join(root, ".relay", "agent.json"), JSON.stringify({ handle: "wren", apiUrl: "https://linked.example" }));
    const configDir = join(root, "config"); mkdirSync(join(configDir, "relay"), { recursive: true });
    writeFileSync(join(configDir, "relay", "config.json"), JSON.stringify({ version: 1, current_profile: "default", profiles: { default: {}, "wren": { agent_token: "rly_linked_abcdefghijklmnop", api_url: "https://linked.example" } } }));
