...
import type Relay, { type RelayWebhookEvent } from "@relaymessenger/sdk";
...
const makeEvent = (id: string, chat: string): RelayWebhookEvent => ({
  ...
  created_at: "2026-01-01T00:00:00Z", data: { direction: "inbound", id: `message-${id}`, chat: { id: chat } as never, sender_handle: { handle: "alice" } as never, parts: [{ type: "text", value: "hello" }] },
});
...
describe("Pi channel", () => {
  ...
  it("times out and stops on abort", async () => {
    const process = fakePi([]); const { relay } = relayFor([makeEvent("timeout", "one")]); const channel = new PiChannel({ agentToken: "secret", relay, rpcTimeoutMs: 5, spawnPi: () => process });
  ...
  it("aborts a live RPC turn and kills its process", async () => {
    const process = fakePi([]); const { relay } = relayFor([makeEvent("abort", "one")]); const controller = new AbortController();
